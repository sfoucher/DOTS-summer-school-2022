...
@EVALUATORS.register()
class MultiChannelsEvaluator(Evaluator):
    ...
    def __init__(self, model: torch.nn.Module, dataloader: torch.utils.data.DataLoader, metrics: Metrics, 
        ...
        stitcher: Optional[Stitcher] = None, work_dir: Optional[str] = None, header: Optional[str] = None
        ) -> None:
        super().__init__(model, dataloader, metrics, device_name=device_name, print_freq=print_freq, 
            stitcher=stitcher, work_dir=work_dir, header=header)
    ...
    def prepare_feeding(self, targets: Dict[str, torch.Tensor], output: torch.Tensor) -> dict:
        ...
        lmds = MultiChannelsLMDS(**self.lmds_kwargs)
...
@EVALUATORS.register()
class ImageLevelEvaluator(Evaluator):
    ...
    def prepare_feeding(self, targets: Dict[str, torch.Tensor], output: torch.Tensor) -> dict:
        ...
        return dict(gt = targets.item(), pred = output[0])