...
@LOSSES.register()
class WHDLoss(WeightedHausdorffDistance):
    ...
    def __init__(
        ...
        super(WHDLoss, self).__init__(
            resized_height=resized_height, resized_width=resized_width,
            p=p, return_2_terms=return_2_terms, device=torch.device(device)
            )
    ...
    def forward(self, output: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        probmap = output.squeeze(1)

        _gt = (target == 1).nonzero(as_tuple=True)
        gt = [[]] * batch_size
        for k, b in enumerate(_gt[0]):
            gt[b] = [*gt[b], torch.stack([_gt[2][k], _gt[3][k]])]

        gt = [torch.stack(b) for b in gt]

        orig_sizes = torch.cat(
            batch_size*[torch.tensor(target.shape[2:]).unsqueeze(0)]
            ).to(self.device)

        res = super().forward(prob_map=probmap, gt=gt, orig_sizes=orig_sizes)
